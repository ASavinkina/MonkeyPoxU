...
studentcontacts = 0 # students quarantined per diagnosed case
...
quarrate = 1/14  # duration of quarantine 
R0_h= 1.4 #R0
...
avg_infectious_plot_i <- ggplot(data=results_all_infected4, aes(x=time2, y=median)) + geom_line() +
  theme_classic() + theme(legend.position = "none") + ylim(0,80) +
...
avg_isolated_plot_i <- ggplot(data=results_all_diagnosed4, aes(x=time2, y=median)) + geom_line() +
  theme_classic() + theme(legend.position = "none") + ylim(0,25) +
...
avg_quarantine_plot_i <- ggplot(data=results_all_quarantined4, aes(x=time2, y=median)) + geom_line() +
  theme_classic() + theme(legend.position = "none") + ylim(0,250) +
...
maxquarplot <- ggplot(results_all_quarantined_likelihood, aes(x=maxQQ)) +
    ...
    theme_classic() + scale_x_continuous(breaks=c(0,50,100,150,200,250), limits = c(0,250)) +
...
infections_plot <- ggplot(total_infections2, aes(x=allinfections)) +
  geom_histogram(fill='grey', color='black') +
  theme_classic() + scale_x_continuous(breaks=c(0,50,100,150,200,250,300,350), limits = c(0,350)) +
  xlab('Cumulative number of infections over 100 days') + ylab('Likelihood')


destination = paste0('Plots/','Plots_',isolation,'_',quarantine,'.pdf')
