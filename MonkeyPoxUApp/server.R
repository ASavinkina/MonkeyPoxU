...
shinyServer(function(input, output) {
    ...
    output$meaninfections_hr <- renderValueBox({
        valueBox(paste(additional_cases_mean_hr,"[",min_infections_hr,",",max_infections_hr,"]"), "Mean [min,max] number infections in 100 days, \ngiven initial case infects others, \nhigh-risk sub-group")
    })
    
    output$meaninfections_lr <- renderValueBox({
      valueBox(paste(additional_cases_mean_lr,"[",min_infections_lr,",",max_infections_lr,"]"), "Mean [min,max] number infections in 100 days, \nlow-risk sub-group")
    })
